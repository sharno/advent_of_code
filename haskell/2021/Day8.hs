...
one _ s = length s == 2
four _ s = length s == 4
seven _ s = length s == 3
eight _ s = length s == 7

output = map (head . drop 1) input

uniqueDigits s = one [] s || four [] s || seven [] s || eight [] s

day8p1 = sum [if uniqueDigits s then 1 else 0 | ss <- output, s <- ss]


-- PART 2

parse _ s | length s == 2 = 1
parse _ s | length s == 4 = 4
parse _ s | length s == 3 = 7
parse _ s | length s == 7 = 8
parse ss s | length s == 5 && (length $ s \\ (fromJust $ find (one []) ss)) == 3 = 3
parse ss s | length s == 5 && (length $ s \\ (fromJust $ find (four []) ss)) == 3 = 2
parse ss s | length s == 5 && (length $ s \\ (fromJust $ find (four []) ss)) == 2 = 5
parse ss s | length s == 6 && (length $ s \\ (fromJust $ find (four []) ss)) == 3 && (length $ s \\ (fromJust $ find (seven []) ss)) == 3 = 0
parse ss s | length s == 6 && (length $ s \\ (fromJust $ find (four []) ss)) == 3  && (length $ s \\ (fromJust $ find (seven []) ss)) == 4 = 6
parse ss s | length s == 6 && (length $ s \\ (fromJust $ find (four []) ss)) == 2 = 9


getL [ss, xs] = [parse ss s | s <- xs]

numListToNum [a,b,c,d] = d + c * 10 + b * 100 + a * 1000

day8p2 = sum $ map (numListToNum . getL) input


input = [    [["dbc","gfecab","afcdg","dfebcag","bd","dgbe","bcaeg","dcefab","ecgadb","agcbd"],["acdgb","gbcda","gdecfba","bacge"]],
